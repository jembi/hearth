...
module.exports = (mongo) => {
  ...
  const afterReadHook = (ctx, data, callback) => {
    populateBinaryContent(data, (err) => {
      ...
      // return in requested format if it matches the defined contentType
      if (ctx.headers['content-type'] === data.contentType) {
        const buf = Buffer.from(data.content, 'base64')

        return callback(null, null, buf)
      }
  ...
  return {
    ...
    searchFilters: (ctx, callback) => {
      ...
      queryValidator.validateQueryParams('Binary', queryParams, customSupportedParams, (badRequest, queryObject) => {
        if (badRequest) {
