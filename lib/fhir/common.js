    ...
    util: {
      ...
      validateAndParseQueryParams: (queryParams, supported, callback) => {
        ...
        for (const key in queryParams) {
          ...
          const keyModifier = keySplit[1] || 'default'
          ...
          if (keyModifier !== 'default' && !supported[keyActual].modifiers[keyModifier]) {
            return callback(`This endpoint has the following query parameter: '${keyActual}' which does not allow for the ':${keyModifier}' modifier`)
          }
