...
module.exports = (mongo) => {
  ...
  return {
    ...
    util: {
      ...
      validateSearchParams: (queryParams, supported) => {
        ...
        for (const key in supported) {
          ...
          if (supported[key].required && !queryParams[key]) {
            const requiredParams = getRequiredParams(supported)
            return `This endpoint has the following required query parameters: ${JSON.stringify(requiredParams)}`
          }
        }
        ...
        for (const key in queryParams) {
          ...
          if (standardFHIRParams.indexOf(key) !== -1) {
            return null
          }

          // parameter not supported
          if (!supported[key]) {
            return `This endpoint does not support the query parameter: ${key}`
          }

          // parameter not allowed to be an array
          if (!supported[key].allowArray && Array.isArray(queryParams[key])) {
            return `This endpoint does not support the query parameter: ${key}`
          }
        }
