    ...
    revertUpdate: (resourceType, id, callback) => {
      mongo.getDB((err, db) => {
        ...
        c.findOne({ _id: ObjectID(id) }, {}, (err, doc) => {
          ...
          let previousLatest = String(Math.max(Object.keys(doc.history).map((key) => { return parseInt(key) })))
          delete doc.latest
          doc.latest = doc.history[previousLatest].resource
          previousLatest === '1' ? delete doc.history : delete doc.history[previousLatest]

          c.update({ _id: ObjectID(id) }, doc, (err, doc) => {
            if (err) {
              return callback(err)
            }
            callback(null, true)
