...
module.exports = (mongo) => {
  ...
  const paramAsDateRangeClause = (fields, params) => {
  ...
  return {
    ...
    util: {
       ...
       *
       * @param {String|Object} fields The FHIR fields that will be matched. The value can either be a single date field (string),
       *                                or it can be a object specifying the start and end fields, if it is a period.
       ...
       */
      paramAsDateRangeClause: paramAsDateRangeClause,
      ...
      genChainedParamQuery: (value, baseParam, chainedParam, chainedModule, callback) => {
        ...
        chainedModule.searchFilters(refQueryParams, (err, badRequest, filters) => {
          ...
          mongo.getDB((err, db) => {
            ...
            c.find(filters).toArray((err, results) => {
              if (err) { return callback(err) }
              if (results.length === 0) {
              ...
              results.forEach((pat) => {
                let clause = {}
                clause[`latest.${baseParam}`] = `${chainedModule.name}/${pat._id}`
                clauses.push(clause)
              })
              if (clauses.length > 1) {
