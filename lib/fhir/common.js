...
module.exports = exports = (mongo) => {
  ...
  function resolveNestedProperties (resultList, includeReference, callback) {
    ...
    const evaluateProperties = (property, nextProperty) => {
      ...
      _.castArray(property).map((prop) => {
        if (nextProperty[0] && prop[nextProperty[0]] != null) {
          ...
          return callback(new Error(`Invalid resource reference: "${JSON.stringify(prop)}"`))
        }
      })
    }
  ...
  const resolveIncludeParameters = (item, results) => {
    return new Promise((resolve, reject) => {
      if (!item) {
        return reject(new Error('Undefined include parameter'))
      }
      const itemList = item.split('.')

      if (itemList.length < 2) {
        return reject(new Error(`Invalid format for include parameter: ${item}`))
      }
    ...
    if (searchParameters) {
      ...
      Promise.all(promises)
        .then((res) => {
          ...
          callback(null, _.uniq(flattenedRes))
        })
    } else {
      callback()
    }
  ...
  function includeResources (includeParams, results) {
    return new Promise((resolve, reject) => {
      ...
      Promise.all(promises).then((res) => {
        ...
        resolve(_.uniqWith(flattenedRes, _.isEqual))
      }).catch((err) => {
        reject(err)
      })
    })
