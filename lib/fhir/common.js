...
module.exports = (mongo) => {
  ...
  return {
    ...
    util: {
      ...
      validateAndModifyQueryParams: (queryParams, supported, callback) => {
        ...
        for (const key in queryParams) {
          ...
          const keyOperator = keySplit[1]

          // if keyOperator supplied, but not allowed in supported settings
          if (keyOperator && !supported[keyActual].operators[keyOperator]) {
            return callback(`This endpoint has the following query parameter: '${keyActual}' which does not allow for the ':${keyOperator}' operator`, null)
          }
          ...
          if (!supported[keyActual]) {
            return callback(`This endpoint does not support the query parameter: ${keyActual}`, null)
          }

          // parameter not allowed to be an array
          if (!supported[keyActual].allowArray && Array.isArray(queryParams[keyActual])) {
            return callback(`This endpoint does not support the query parameter to be in array format: ${keyActual}`, null)
          }

          queryObject[keyActual] = {
            value: queryParams[key],
            operators: supported[keyActual].operators
          }
        }
        ...
        for (const key in supported) {
          ...
          if (supported[key].required && !queryObject[key]) {
            ...
            return callback(`This endpoint has the following required query parameters: ${JSON.stringify(requiredParams)}`, null)
          }
        }
