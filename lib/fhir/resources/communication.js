  ...
  return {
    ...
    searchFilters: (ctx, callback) => {
      ...
      queryUtils.validateAndParseQueryParams(queryParams, supportedParams, (badRequest, queryObject) => {
        ...
        if (queryObject['sent']) {
          query['$and'].push(queryUtils.dateToMongoClause('sent', queryObject['sent'][constants.NO_MODIFER]))
        }

        if (queryObject['received']) {
          query['$and'].push(queryUtils.dateToMongoClause('received', queryObject['received'][constants.NO_MODIFER]))
        }
