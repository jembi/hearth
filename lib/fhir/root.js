...
module.exports = (mongo, modules, callback) => {
  ...
  const revertTransaction = (ctx, revert) => {
    const promises = []

    const revertOperations = (operationString, revertFunctionString) => {
      revert[operationString].forEach((operation) => {
        promises.push(new Promise((resolve, reject) => {
          transaction[revertFunctionString](ctx, operation.resourceType, operation.id, (err, success) => {
    ...
    async.series(entryFunctions, (err, resEntries) => {
      if (err) {
        ...
        if (isTransaction) {
          ...
          promises = revertTransaction(ctx, revert)
        }
  ...
  return {
    ...
    /**
     * Pulls updates from the given URL and then updates them in the system.
     *
     * @param {RequestContext} ctx
     * @param {String} url
     * @param {CoreCallback} callback
        ...
        var req = http.request( options, (res) => {
            ...
            res.on('end', () => {
                if ( body ) {
                  try {
                    console.log(body);
                    var bundle = JSON.parse(body)
                    bundle['entry'].forEach( (entry) => {
                      fhirCore.update( ctx, entry['resource']['resourceType'], entry['resource']['id'], entry['resource'], (err) => {
                        if ( err ) logger.warn(err)
                      })
                    })
                    return callback( null, fhirCommon.buildHTTPOutcome( 200, 'information', 'informational', 'Updates are being processed and loaded.' ) )
                  } catch( err ) {
                    return callback( null, fhirCommon.buildHTTPOutcome( 200, 'error', 'invalid', 'Invalid data returned from '+ctx.query.url ) )
                  }
                } else {
                  return callback( null, fhirCommon.buildHTTPOutcome( 200, 'error', 'invalid', 'No data returned from '+ctx.query.url ) )
                }
            })
            res.on('error', (err) => {
                return callback(err)
