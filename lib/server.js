...
let stop = (killSignal, callback) => {
  if (typeof killSignal === 'function') {
    callback = killSignal
    killSignal = undefined
  }
  Object.keys(workers).forEach((key) => {
    workers[key].kill(killSignal)
  })
  server.close(() => {
...
if (!module.parent) {
  start(() => {
    user.loadDefaultSysadmin((err) => {
      ...
      process.on('SIGINT', () => stop('SIGINT', process.exit))
      // terminate signal
      process.on('SIGTERM', () => stop('SIGTERM', process.exit))
    })
  })
}
