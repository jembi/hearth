...
const processQueueElement = (msg, callback) => {
  matching.match(msg.payload.resourceType, msg.payload, matchingConfig.matchSettings.defaultCount, (err, badRequest, matchesArray, certainMatchFlag) => {
    if (err) {
      return callback(err)
    }

    const referenceLink = `${msg.payload.resourceType}/${msg.payload.id}`
    resourceLinking.addLinkReferenceToMatches(matchesArray, referenceLink, (err) => {
      if (err) {
        return callback(err)
      }
