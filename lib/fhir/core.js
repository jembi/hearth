        ...
        hooks.executeBeforeHooks('hierarchy', fhirModule, ctx, resourceType, (err, badRequest) => {
          ...
          mongo.getDB((err, db) => {
            ...
            if ( ctx.operation === '$parents' ) {
                query.push( { $graphLookup: { from: "LocationRef", startWith: "$refId", connectFromField: "partOf.reference", connectToField: "refId", as: "hierarchy" } } ) 
            ...
            db.listCollections({name:viewCollection}).toArray((err,results) => {
              ...
              if ( results.length == 0 ) {
                db.command( { create: viewCollection, viewOn: resourceType, 
                    pipeline: [ { $addFields: { refId: { $concat: [ "$resourceType", "/", "$id" ] } } } ] }, (err, results ) => {
                  if ( err ) {
                    return callback(err)
                  }
