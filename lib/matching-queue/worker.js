...
const processQueueElement = (msg, callback) => {
  matching.match(msg.payload.resourceType, msg.payload, 10, (err, badRequest, matchesArray, certainMatchFlag) => {
    ...
    if (badRequest) {
      return callback(null, fhirCommon.buildOperationOutcome(400, 'error', 'invalid', badRequest))
    }

    const referenceLink = `${msg.payload.resourceType}/${msg.payload.id}`
    resourceLinking.addLinkReferenceToMatches(matchesArray, referenceLink, (err, badRequest) => {
      if (err) {
        return callback(err)
      }

      if (badRequest) {
        return callback(null, fhirCommon.buildOperationOutcome('error', 'invalid', 'Must specify \'system\' and \'code\' parameters'))
      }
