    ...
    searchFilters: (ctx, callback) => {
      ...
      queryUtils.validateAndParseQueryParams(queryParams, supportedParams, (badRequest, queryObject) => {
        ...
        if (queryObject['practitioner.organization']) {
          fhirCommon.lookupPractitionersForOrganization(ctx, queryUtils.paramAsReference(queryObject['practitioner.organization'][constants.NO_MODIFER], 'Organization'), (err, practitioners) => {
            if (err) {
              return callback(err)
            }
            build(practitioners)
