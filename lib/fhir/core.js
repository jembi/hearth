      ...
      authorization.authorize('vread', ctx, resourceType, (err, badRequest, postInteractionHandler) => {
        ...
        const processVRead = (err, badRequest) => {
          ...
          mongo.getDB((err, db) => {
            ...
            c.findOne({ id: id, _versionId: versionId }, (err, resource) => {
              if (err) {
                return callback(err)
              }
              ...
              if (!resource) {
                ...
                cHistory.findOne({ id: id, _versionId: versionId }, (err, resource) => {
                  if (err) {
                    return callback(err)
                  }
                  if (!resource) {
            ...
            const onDone = () => {
              ...
              fhirCommon.getPagingParams(ctx.query, (err, badRequest, _getpagesoffset, _count) => {
                ...
                c.count(query, (err, total) => {
                  ...
                  c.find(query).skip(_getpagesoffset).limit(_count).toArray((err, results) => {
                    if (err) {
                      return callback(err)
                    }
            ...
            c.insertOne(resource, (err, r) => {
              ...
              postInteractionHandler(ctx, resource, (err, badRequest) => {
                ...
                const done = (err, badRequest) => {
                  ...
                  const location = `/fhir/${resourceType}/${resource.id}/_history/${resource._versionId}`
      ...
      authorization.authorize('update', ctx, resourceType, (err, badRequest, postInteractionHandler) => {
        ...
        const processUpdate = (err, badRequest) => {
          ...
          mongo.getDB((err, db) => {
            ...
            const c = db.collection(resourceType)
            c.findOneAndUpdate({ id: id }, update, options, (err, result) => {
              if (err) {
                return callback(err)
              }
              ...
              cHistory.insert(result.value, (err) => {
                ...
                postInteractionHandler(ctx, resource, (err, badRequest) => {
                  ...
                  const done = (err, badRequest) => {
                    ...
                    const location = `/fhir/${resourceType}/${id}/_history/${result.value._versionId + 1}`
