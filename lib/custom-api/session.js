...
function generateTokenForUser (user, callback) {
  ...
  if (Object.keys(jwtConf).length === 0 && jwtConf.constructor === Object) { // if object empty - keep backwards compatibility of config
    secretOrPrivateKey = config.getConf('authentication:secret')
    console.log('secret is...:', secretOrPrivateKey)
    ...
    secretOrPrivateKey = jwtConf.privKey
  } else {
