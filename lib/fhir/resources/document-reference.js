...
module.exports = (mongo) => {
  ...
  const transformHandler = (req, res, onSuccess) => {
    ...
    if (req.body.context && req.body.context.period) {
      req.body._transforms.context = { period: {} }
      if (req.body.context.period.start) {
        req.body._transforms.context.period.start = fhirCommon.util.transformDate(req.body.context.period.start)
      }
      if (req.body.context.period.end) {
        req.body._transforms.context.period.end = fhirCommon.util.transformDate(req.body.context.period.end)
      }
    }
    onSuccess()
  }
