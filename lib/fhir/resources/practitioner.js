    ...
    searchFilters: (ctx, callback) => {
      ...
      fhirCommon.util.validateAndParseQueryParams(queryParams, supportedParams, (badRequest, queryObject) => {
        ...
        if (queryObject['given']) {
          query['$and'].push(fhirCommon.util.nameToMongoClause('given', queryObject['given'][constants.NO_MODIFER]))
        }

        if (queryObject['family']) {
          query['$and'].push(fhirCommon.util.nameToMongoClause('family', queryObject['family'][constants.NO_MODIFER]))
        }

        if (queryObject['role']) {
          query['$and'].push(fhirCommon.util.tokenToSystemCodeElemMatch('practitionerRole.role.coding', queryObject['role'][constants.NO_MODIFER]))
        }
          ...
          clause['practitionerRole'] = {
            $elemMatch: {
              managingOrganization: {
                reference: fhirCommon.util.paramAsReference(queryObject['organization'][constants.NO_MODIFER], 'Organization')
              }
            }
          }
