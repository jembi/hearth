...
app.get('/fhir/:resourceType/:id', (req, res, next) => {
  ...
  } else if ( (res.locals.ctx.operation === "$hierarchy" || res.locals.ctx.operation === '$parents' ) 
      && req.params.resourceType === "Location" ) {
    fhirCore.hierarchy(res.locals.ctx, req.params.resourceType, req.params.id, buildCoreCallbackHandler(req, res, next))
  } else {
...
app.post('/csd', (req, res, next) => {
  var soapReq = libxmljs.parseXml(req.body);
  var lastModified = soapReq.get("//csd:getModificationsRequest/csd:lastModified", {
...
let start = (callback) => {
  server = app.listen(config.getConf('server:port'), config.getConf('server:hostname'), () => {
    ...
    matchingQueue.startQueueWorkers(workers, config.getConf('matchingQueue:numberOfWorkers'), config.getConf("matchingQueue:extraDatabases"), (err) => {
      if (err) {
