    ...
    searchFilters: (ctx, callback) => {
      ...
      fhirCommon.util.validateAndModifyQueryParams(queryParams, supportedParams, (badRequest, queryObject) => {
        ...
        if (queryObject['indexed']) {
          for (const k in queryObject['indexed']) {
            query['$and'].push(fhirCommon.util.paramAsDateRangeClause('_transforms.indexed', queryObject['indexed'][k]))
          }
        }
        ...
        if (queryObject['class']) {
          for (const k in queryObject['class']) {
            query['$and'].push(fhirCommon.util.tokenToSystemCodeElemMatch('class.coding', queryObject['class'][k]))
          }
        }
        ...
        if (queryObject['setting']) {
          for (const k in queryObject['setting']) {
            query['$and'].push(fhirCommon.util.tokenToSystemCodeElemMatch('context.practiceSetting.coding', queryObject['setting'][k]))
          }
        }

        if (queryObject['period']) {
          for (const k in queryObject['period']) {
            query['$and'].push(fhirCommon.util.paramAsDateRangeClause({
              ...
              queryObject['period'][k]
            ))
          }
        }

        if (queryObject['facility']) {
          for (const k in queryObject['facility']) {
            query['$and'].push(fhirCommon.util.tokenToSystemCodeElemMatch('context.facilityType.coding', queryObject['facility'][k]))
          }
        }

        if (queryObject['event']) {
          for (const k in queryObject['event']) {
            query['$and'].push(fhirCommon.util.tokenToSystemCodeElemMatch('context.event.coding', queryObject['event'][k]))
          }
        }

        if (queryObject['securityLabel']) {
          for (const k in queryObject['securityLabel']) {
            query['$and'].push(fhirCommon.util.tokenToSystemCodeElemMatch('securityLabel.coding', queryObject['securityLabel'][k]))
          }
        }

        if (queryObject['format']) {
          for (const k in queryObject['format']) {
            query['$and'].push(fhirCommon.util.tokenToSystemCodeElemMatch('content.format', queryObject['format'][k]))
          }
        }

        if (queryObject['related-id']) {
          for (const k in queryObject['related-id']) {
            query['$and'].push(fhirCommon.util.tokenToSystemValueElemMatch('context.related.identifier', queryObject['related-id'][k]))
          }
        }
