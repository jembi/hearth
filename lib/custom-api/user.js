...
module.exports = (mongo) => {
  ...
  return {
    ...
    read: (req, res) => {
      authorization.authorize('read', res.locals.ctx, 'user', (err, badRequest, postInteractionHandler) => {
        if (err) {
          logger.error(err)
          return res.status(500).send(fhirCommon.internalServerErrorOutcome())
        }
        ...
        mongo.getDB((err, db) => {
          ...
          c.findOne({email: req.params.email}, {fields: {email: 1, type: 1, resource: 1}}, (err, user) => {
            ...
            postInteractionHandler(res.locals.ctx, user, (err, badRequest, user) => {
              if (err) {
                logger.error(err)
                return res.status(500).send(fhirCommon.internalServerErrorOutcome())
              }
    ...
    search: (req, res) => {
      authorization.authorize('search', res.locals.ctx, 'user', (err, badRequest, postInteractionHandler, searchFilters) => {
        if (err) {
          logger.error(err)
          return res.status(500).send(fhirCommon.internalServerErrorOutcome())
        }
    ...
    create: (req, res) => {
      authorization.authorize('create', res.locals.ctx, 'user', (err, badRequest, postInteractionHandler) => {
        if (err) {
          logger.error(err)
          return res.status(500).send(fhirCommon.internalServerErrorOutcome())
        }
        ...
        mongo.getDB((err, db) => {
          ...
          c.findOne({email: req.body.email}, {fields: {email: 1}}, (err, user) => {
            ...
            c.insertOne(req.body, (err) => {
              ...
              postInteractionHandler(res.locals.ctx, req.body, (err, badRequest) => {
                if (err) {
                  logger.error(err)
                  return res.status(500).send(fhirCommon.internalServerErrorOutcome())
                }
    ...
    update: (req, res) => {
      authorization.authorize('update', res.locals.ctx, 'user', (err, badRequest, postInteractionHandler) => {
        if (err) {
          logger.error(err)
          return res.status(500).send(fhirCommon.internalServerErrorOutcome())
        }
