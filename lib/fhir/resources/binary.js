...
module.exports = (mongo) => {
  ...
  const afterReadHook = (ctx, data, callback) => {
    populateBinaryContent(ctx, data, (err) => {
      if (err) {
        return callback(err)
      }
  ...
  const afterSearchHook = (ctx, data, callback) => {
    ...
    data.entry.forEach((resource) => {
      ...
      promises.push(new Promise((resolve, reject) => {
        populateBinaryContent(ctx, resource.resource, (err) => {
          if (err) {
            return reject(err)
          }
          resolve()
  ...
  const populateBinaryContent = (ctx, data, callback) => {
    mongo.getDB((err, db) => {
      if (err) {
        return callback(err)
      }
  ...
  const writeToGridFS = (ctx, resource, done) => {
    mongo.getDB((err, db) => {
      if (err) {
        return done(err)
      }
  ...
  return {
    ...
    before: {
      create: (ctx, resource, done) => {
        writeToGridFS(ctx, resource, done)
      },
