...
module.exports = (fhirCommon) => {
  ...
  return {
    buildPIXmQuery: (ctx, callback) => {
      ...
      const supportedParams = ['sourceIdentifier', 'targetSystem']
      const requiredParams = ['sourceIdentifier']

      let badRequest = fhirCommon.util.validateSearchParams(queryParams, supportedParams, requiredParams) ||
    ...
    searchPostInteractionHandler: (ctx, data, callback) => {
      ...
      if (data[0].link && data[0].link.length > 0) {
        data[0].link.forEach((link) => {
          queriedUrlFiltered = matchUrlToSourceIdentifier(link.other, sourceIdentifier) ?  true : false
      ...
      queriedUrlFiltered = matchUrlToSourceIdentifier(patientUrl, sourceIdentifier) ?  true : false
