    ...
    searchFilters: (ctx, callback) => {
      ...
      fhirCommon.util.validateAndParseQueryParams(queryParams, supportedParams, (badRequest, queryObject) => {
        ...
        if (queryObject['birthDate']) {
          query['$and'].push(fhirCommon.util.dateToMongoClause('birthDate', queryObject['birthDate'][constants.NO_MODIFER]))
        }
        ...
        if (queryObject['telecom']) {
          query['$and'].push(fhirCommon.util.tokenToSystemValueElemMatch('telecom', queryObject['telecom'][constants.NO_MODIFER]))
        }
