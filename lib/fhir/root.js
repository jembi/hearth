...
module.exports = (mongo, modules, callback) => {
  ...
  const buildOutcomeHandler = (entry, method, resourceType, revert, isTransaction, callback) => {
    return (err, outcome) => {
      ...
      if (outcome.httpStatus && isTransaction >= 500) {
  ...
  const processEntry = (baseCtx, entry, revert, isTransaction, callback) => {
    ...
    let entryCtx = _.cloneDeep(baseCtx)
    ...
    switch (entryCtx.method) {
      case 'DELETE':
        ...
        break
      case 'POST':
        fhirCore.create(entryCtx, resourceType, entry.resource, entry._id, buildOutcomeHandler(entry, 'POST', resourceType, revert, isTransaction, callback))
        break
      case 'PUT':
  ...
  const processBundle = (ctx, bundle, isTransaction, callback) => {
    ...
    // process entry function in order
