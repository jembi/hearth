  ...
  let buildSearchFilters = (queryObject, practitionersToFilter) => {
    ...
    if (queryObject['practitioner']) {
      query['$and'].push({
        participant: {
          $elemMatch: {
            'individual.reference': fhirCommon.util.paramAsReference(queryObject['practitioner'][constants.NO_MODIFER], 'Practitioner')
          }
        }
      })
    }
    ...
    if (queryObject['participant']) {
      query['$and'].push({
        participant: {
          $elemMatch: {
            'individual.reference': fhirCommon.util.paramAsReference(queryObject['participant'][constants.NO_MODIFER], 'Practitioner')
          }
        }
      })
    }
    ...
    if (queryObject['location']) {
      query['$and'].push({
        location: {
          $elemMatch: {
            'location.reference': fhirCommon.util.paramAsReference(queryObject['location'][constants.NO_MODIFER], 'Location')
          }
        }
      })
    }
    ...
    searchFilters: (ctx, callback) => {
      ...
      fhirCommon.util.validateAndParseQueryParams(queryParams, supportedParams, (badRequest, queryObject) => {
        ...
        if (queryObject['practitioner.organization']) {
          fhirCommon.lookupPractitionersForOrganization(fhirCommon.util.paramAsReference(queryObject['practitioner.organization'][constants.NO_MODIFER], 'Organization'), (err, practitioners) => {
            if (err) {
              return callback(err)
            }
            build(practitioners)
