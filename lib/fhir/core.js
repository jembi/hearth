...
module.exports = (mongo, modules) => {
  ...
  return {
                ...
                c.count(query, projection, (err, total) => {
                  ...
                  c.find(query, projection).skip(_getpagesoffset).limit(_count).toArray((err, results) => {
                    ...
                    const includesPromise = fhirCommon.mapSearchNameToPath(ctx.query._include).then((paths) => {
                      return fhirCommon.includeResources(ctx, paths, results)
                    })

                    const revincludesPromise = fhirCommon.reverseIncludeResources(ctx, results)
    ...
    /**
     * Hierarchy or Parents for a Location.  Return a bundle of all children/grandchildren/etc. under a Location.
     * Requires a view to resolve partOf references.
     * db.createView( "LocationRef", "Location", [ { $addFields: { refId: { $concat: [ "$resourceType", "/", "$id" ] } } } ] )
     *
     * @param {RequestContext} ctx
     * @param {String} resourceType The type of FHIR resource
     * @param {CoreCallback} callback
     */
    hierarchy: (ctx, resourceType, id, callback) => {
      authorization.authorize('hierarchy', ctx, resourceType, (err, badRequest) => {
        if (err || badRequest) {
          return handleErrorAndBadRequest(err, badRequest, callback)
        }

        const fhirModule = modules[resourceType]
        hooks.executeBeforeHooks('hierarchy', fhirModule, ctx, resourceType, (err, badRequest) => {
          if (err || badRequest) {
            return handleErrorAndBadRequest(err, badRequest, callback)
          }
          ...
          mongo.getDB((err, db) => {
            ...
            if ( ctx.operation === '$parents' ) {
                query.push( { $graphLookup: { from: "LocationRef", startWith: "$id", connectFromField: "parentId", connectToField: "id", as: "hierarchy" } } ) 
                query.push( { $unwind: "$hierarchy" } )
                query.push( { $replaceRoot: { newRoot: "$hierarchy" } } )
            } else {
                query.push( { $lookup: { from: "LocationRef", localField: "_id", foreignField: "_id", as: "me" } } )
                query.push( { $graphLookup: { from: "LocationRef", startWith: "$refId", connectFromField: "refId", connectToField: "partOf.reference", as: "hierarchy" } } )
                query.push( { $project: { all: { $setUnion: [ "$me", "$hierarchy" ] } } } )
                query.push( { $unwind: "$all" } )
                query.push( { $replaceRoot: { newRoot: "$all" } } )
            }

            const viewCollection = resourceType+"Ref"

            mongo.util.debugLog(viewCollection, 'hierarchy', query)

            const runQuery = ( db, viewCollection, query, callback ) => {

              let cntQuery = query.slice(0,query.length)
              cntQuery.push( {$count: 'total'} )

              const c = db.collection(viewCollection)

              c.aggregate(cntQuery, { allowDiskUse: true }, (err, total) => {
                if (err) {
                  return callback(err)
                }
                if ( total.length == 0 ) {
                    const response = fhirCommon.bundleResults('searchset', [], 0, ctx.url)
                    hooks.executeAfterHooks('hierarchy', fhirModule, ctx, resourceType, response, (err, badRequest, results) => {
                      if (err || badRequest) {
                        return handleErrorAndBadRequest(err, badRequest, callback)
                      }
                  ...
                  c.aggregate(query, { allowDiskUse: true } ).toArray((err, results) => {
                    if (err) {
                      return callback(err)
                    }
                    ...
                    hooks.executeAfterHooks('hierarchy', fhirModule, ctx, resourceType, response, (err, badRequest, results) => {
                      ...
                      if (results && results.entry) {
                        results.entry.forEach((resource) => {
                          delete resource.resource.refId
                          fhirCommon.formatResource(resource.resource)
                        })
                      }
            ...
            db.listCollections({name:viewCollection}).toArray((err,results) => {
              if ( err ) {
                return callback(err)
              }
              if ( results.length == 0 ) {
                db.command( { create: viewCollection, viewOn: resourceType, 
                    pipeline: [ { $addFields: { refId: { $concat: [ "$resourceType", "/", "$id" ] }, 
                        parentId: { $substr: [ "$partOf.reference", 9, -1 ] } } } ] }, (err, results ) => {
                  if ( err ) {
                    return callback(err)
                  }
                  runQuery( db, viewCollection, query, callback )
                })
              } else {
                runQuery( db, viewCollection, query, callback )
              }
