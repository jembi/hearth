    ...
    searchFilters: (ctx, callback) => {
      ...
      fhirCommon.util.validateAndParseQueryParams(queryParams, supportedParams, (badRequest, queryObject) => {
        ...
        if (queryObject['questionnaire']) {
          for (const modifier in queryObject['questionaire']) {
            query['$and'].push({ 'questionnaire.reference': fhirCommon.util.paramAsReference(queryObject['questionnaire'][modifier], 'Questionnaire') })
          }
        }
