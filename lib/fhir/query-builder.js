...
module.exports = (mongo) => {
  ...
  const buildQueryForNumber = (path, propertyTypeObj, modifier, value) => {
    throw new Error('Unsupported parameter type - number')
  }

  const buildQueryForDate = (path, propertyTypeObj, modifier, value) => {
    return queryUtils.dateToMongoClause(path, value)
  }

  const buildQueryForString = (path, propertyTypeObj, modifier, value) => {
  ...
  const buildQueryForURI = (path, propertyTypeObj, modifier, value) => {
  ...
  const buildQueryForToken = (path, propertyTypeObj, modifier, value) => {
  ...
  const buildQueryForQuantity = (path, propertyTypeObj, modifier, value) => {
    throw new Error('Unsupported parameter type - quantity')
  }

  const buildQueryForReference = (path, propertyTypeObj, modifier, value) => {
      ...
      searchParams.forEach((searchParam) => {
        ...
        for (let modifier in queryObject[queryParam]) {
          ...
          propertyDef.types.forEach((typeObj) => {
            typeQuery['$or'].push(queryBuilderFuncs[searchParam.type](searchParam.path, typeObj, modifier, queryObject[queryParam][modifier]))
          })
