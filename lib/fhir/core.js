...
module.exports = (mongo, modules) => {
  ...
  return {
    ...
    /**
     * Hierarchy for a Location.  Return a bundle of all children/grandchildren/etc. under a Location.
      ...
      authorization.authorize('hierarchy', ctx, resourceType, (err, badRequest) => {
        ...
        hooks.executeBeforeHooks('hierarchy', fhirModule, ctx, resourceType, (err, badRequest) => {
          ...
          mongo.getDB((err, db) => {
            ...
            let query = [
              { $match: { id: id } },
              { $graphLookup: { from: "LocationRef", startWith: "$refId", connectFromField: "refId", connectToField: "partOf.reference", as: "hierarchy" } },
              { $unwind: "$hierarchy" }, 
            ...
            mongo.util.debugLog(resourceType+"Ref", 'hierarchy', query)

            const c = db.collection(resourceType+"Ref")
            c.aggregate(cntQuery, (err, total) => {
              if (err) {
                return callback(err)
              }
