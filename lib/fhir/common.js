...
module.exports = exports = (mongo) => {
      ...
      if (Array.isArray(property)) {
        for (var n = 0; n < property.length; n++) {
          if (nextProperty[0] && property[n][nextProperty[0]] != null) {
            evaluateProperties(property[n][nextProperty[0]], nextProperty.slice(1))
          } else if (property[n].reference) {
            promises.push(new Promise((resolve, reject) => {
              resolveReference(property[n].reference, (err, data) => {
                if (err) {
                  reject(err)
                } else {
                  resolve(data)
        ...
        if (nextProperty[0] && property[nextProperty[0]] != null) {
          evaluateProperties(property[nextProperty[0]], nextProperty.slice(1))
        } else if (property.reference) {
          promises.push(new Promise((resolve, reject) => {
            resolveReference(property.reference, (err, data) => {
              if (err) {
                reject(err)
              } else {
                resolve(data)
  ...
  const parseSearchExpression = (exp, callback) => {
    const itemList = exp.split(':')
    const itemResource = itemList[0]
    const itemReference = itemList[1].split('.')

    callback(itemResource, itemReference)
  }
  ...
  function mapSearchNameToPath (searchParameters, callback) {
    const promises = []
    if (searchParameters) {
      if (Array.isArray(searchParameters)) {
        searchParameters.forEach((parameter) => {
          promises.push(new Promise((resolve, reject) => {
            parseSearchExpression(parameter, (resource, reference) => {
              let searchPath = searchParamMappings.nameToPath[resource][reference]
        ...
        promises.push(new Promise((resolve, reject) => {
          parseSearchExpression(searchParameters, (resource, reference) => {
            let searchPath = searchParamMappings.nameToPath[resource][reference]
      ...
      Promise.all(promises)
        .then((res) => {
          const flattenedRes = [].concat.apply([], res)
          callback(null, flattenedRes)
        })
    } else {
      callback('No _include search parameters supplied.')
    }
  }
  ...
  function includeResources (includeParams, results) {
    return new Promise((resolve, reject) => {
      ...
      if (!Array.isArray(results)) {
        return reject(`Invalid results parameter "${results}"`)
      }
      ...
      if (Array.isArray(includeParams)) {
        ...
        promises.push(resolveIncludeParameters(includeParams, results))
      }
      ...
      Promise.all(promises).then((res) => {
        ...
        for (var i = flattenedRes.length - 1; i >= 0; i--) {
          if (flattenedRes[i] == null) {
            flattenedRes.splice(i, 1) // remove all null values from the result set
          }
        }
