    ...
    mongo.getDB((err, db) => {
      ...
      bucketStream.on('end', () => {
        try {
          ...
          fhirRoot.processRootBundle(ctx, jsonObject, (err, httpResponse) => {
            if (err) {
              logger.error(err)
              return callback()
            }

            if (!(httpResponse.httpStatus >= 200 < 300)) {
              logger.error(`Non 2xx status code: ${httpResponse.httpStatus} while trying to process binary bundle`)
              return callback()
            }
