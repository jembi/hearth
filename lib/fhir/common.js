...
module.exports = exports = (mongo) => {
      ...
      _.castArray(property).map((prop) => {
        if (nextProperty[0] && prop[nextProperty[0]] != null) {
          ...
          promises.push(new Promise((resolve, reject) => {
            if (!prop.reference) {
              return reject(new Error(`Invalid resource reference: "${JSON.stringify(prop)}"`))
            }
  ...
  function performReverseInclude (revinclude, results) {
    ...
    return Promise.all(queryPromises).then((queries) => {
      const getDB = util.promisify(mongo.getDB)
      return getDB().then(db => db.collection(resourceType).find({$or: queries}).toArray())
    })
  }
