...
const incomingAcceptsContentTypeHandler = (req, res, next) => {
  const accepts = req.accepts(['application/json+fhir', 'application/json', 'application/xml+fhir', 'application/xml', 'text/xml'])
...
const contentTypeNegotiation = (req, res, next) => {
  ...
  if (req.rawBody && req.get('content-type') && ['application/xml+fhir', 'application/xml', 'text/xml'].includes(req.get('content-type'))) {
...
const outcomeHandler = (req, res, next) => {
  ...
  if (res.locals.outcome.resource) {
    ...
    if (['application/xml+fhir', 'application/xml', 'text/xml'].includes(res.get('content-type'))) {
      const fhir = new FHIR(FHIR.DSTU2)
