...
module.exports = (mongo) => {
  ...
  const addLinkReferenceToMatches = (matchesBundleEntries, referenceLink, callback) => {
      ...
      switch (matchEntry.search.extension.valueCode) {
        case 'certain':
          promises.push(new Promise((resolve, reject) => {
            try {
              const resource = linkResource(matchEntry.resource, referenceLink, constants.LINK_TYPE_CERTAIN_DUPLICATE_SOURCE, matchEntry.search.score)
        ...
        case 'probable':
          promises.push(new Promise((resolve, reject) => {
            try {
              const resource = linkResource(matchEntry.resource, referenceLink, constants.LINK_TYPE_PROBABLE_DUPLICATE_SOURCE, matchEntry.search.score)
              updateResource(resource, createPromiseErrHandlerForCallback(resolve, reject))
            } catch (err) {
              logger.debug(err)
              resolve() // ignore error where the link already exists
        ...
        case 'possible':
          promises.push(new Promise((resolve, reject) => {
            try {
              const resource = linkResource(matchEntry.resource, referenceLink, constants.LINK_TYPE_POSSIBLE_DUPLICATE_SOURCE, matchEntry.search.score)
              updateResource(resource, createPromiseErrHandlerForCallback(resolve, reject))
            } catch (err) {
              logger.debug(err)
              resolve() // ignore error where the link already exists
