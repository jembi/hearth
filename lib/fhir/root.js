...
module.exports = (mongo, modules, callback) => {
  ...
  const processBundle = (ctx, bundle, isTransaction, callback) => {
      ...
      promises.push(new Promise((resolve, reject) => {
        ...
        switch (entryCtx.method) {
          case 'DELETE':
            ...
            break
          case 'POST':
            fhirCore.create(entryCtx, resourceType, entry.resource, buildOutcomeHandler('POST', resourceType))
            break
          case 'PUT':
    ...
    Promise.all(promises).then(() => {
      ...
      if (isTransaction) {
        // Check responses for errors and revert
          ...
          revert.creates.forEach((create) => {
            promises.push(new Promise((resolve, reject) => {
              transaction.revertCreate(create.resourceType, create.id, (err, success) => {
                ...
                if (!success) { logger.warn(`Transaction ${bundle.id} revert failed: counld\'t revert creation of ${create.resourceType}/${create.id}`) }
                resolve()
              })
            }))
          })
          ...
          revert.updates.forEach((update) => {
            promises.push(new Promise((resolve, reject) => {
              transaction.revertUpdate(update.resourceType, update.id, (err, success) => {
                if (err) { return reject(err) }
                if (!success) { logger.warn(`Transaction ${bundle.id} revert failed: counld\'t revert update of ${update.resourceType}/${update.id}`) }
                resolve()
              })
            }))
          })
