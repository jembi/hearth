      ...
      authorization.authorize('hierarchy', ctx, resourceType, (err, badRequest) => {
        ...
        hooks.executeBeforeHooks('hierarchy', fhirModule, ctx, resourceType, (err, badRequest) => {
          ...
          mongo.getDB((err, db) => {
            ...
            let lookup = { from: "LocationRef", startWith: "$refId", connectFromField: "refId", connectToField: "partOf.reference", as: "hierarchy" } 
            if ( ctx.operation === '$parents' ) {
              lookup = { from: "LocationRef", startWith: "$partOf.reference", connectFromField: "partOf.reference", connectToField: "refId", as: "hierarchy" } 
            }
