...
module.exports = (mongo) => {
  return {
    ...
    // no elemMatch for when searching within an object
    ...
    // include elemMatch for when searching within an array
    ...
    genChainedParamQuery: (value, baseParam, chainedParam, chainedModule, callback) => {
      ...
      if (Array.isArray(chainedModule)) {
        const promises = []

        for (const module in chainedModule) {
          let param = chainedParam

          if (Array.isArray(param)) {
            param = chainedParam[module]
          }

          promises.push(new Promise((resolve, reject) => {
            fetchReferencedFilters(param, chainedModule[module], (err, clauses) => {
              if (err) {
                return reject(err)
              }
        ...
        Promise.all(promises).then((clauses) => {
          const multipleResourceClauses = {
            '$or': clauses
          }

          callback(null, multipleResourceClauses)
        }).catch((err) => {
          callback(err)
        })
      } else {
        ...
        if (Array.isArray(param)) {
          param = chainedParam[0] // only one chainedModule, use first array index
        }

        fetchReferencedFilters(param, chainedModule, (err, clauses) => {
          callback(err, clauses)
        })
      }
    },
