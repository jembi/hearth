      ...
      fhirCommon.util.validateAndModifyQueryParams(queryParams, supportedParams, (badRequest, queryObject) => {
        ...
        if (queryObject['given']) {
          for (const k in queryObject['given']) {
            query['$and'].push(fhirCommon.util.nameToMongoClause('given', queryObject['given'][k]))
          }
        }

        if (queryObject['family']) {
          for (const k in queryObject['family']) {
            query['$and'].push(fhirCommon.util.nameToMongoClause('family', queryObject['family'][k]))
          }
        }

        if (queryObject['role']) {
          for (const k in queryObject['role']) {
            query['$and'].push(fhirCommon.util.tokenToSystemCodeElemMatch('practitionerRole.role.coding', queryObject['role'][k]))
          }
        }

        if (queryObject['organization']) {
          for (const k in queryObject['organization']) {
            let clause = {}
            clause['practitionerRole'] = {
              $elemMatch: {
                managingOrganization: {
                  reference: fhirCommon.util.paramAsReference(queryObject['organization'][k], 'Organization')
                }
              }
            }
