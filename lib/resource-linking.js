...
module.exports = (mongo) => {
  ...
  const addLinkToMatches = (matchesBundleEntries, resource, callback) => {
    logger.debug('Processing matched results for linking')

    const promises = []
    matchesBundleEntries.forEach((match) => {
  ...
  const addLinkToMatchAndUpdate = (match, resource, linkType) => {
    return new Promise((resolve, reject) => {
      ...
      try {
        const resource = addLinkToResource(match, matchLink, linkType, match._mpi.search.score)
  ...
  const linkMatchToResource = (match, resource, linkType) => {
    return new Promise((resolve, reject) => {
      const matchLink = `${match.resourceType}/${match.id}`
      try {
        addLinkToResource(resource, matchLink, linkType, match._mpi.search.score, createPromiseErrHandlerForCallback(resolve, reject))
      } catch (err) {
        logger.debug(err)
        resolve() // ignore error where the link already exists
      }
  ...
  const addMatchesLinksToResource = (mathcesArray, resource, callback) => {
    mathcesArray.forEach((match) => {
      switch (match._mpi.search.extension.valueCode) {
        case 'certain':
          linkMatchToResource(match, resource, constants.LINK_TYPE_CERTAIN_DUPLICATE_TARGET)
          break
        case 'probable':
          linkMatchToResource(match, resource, constants.LINK_TYPE_PROBABLE_DUPLICATE_TARGET)
          break
        case 'possible':
          linkMatchToResource(match, resource, constants.LINK_TYPE_POSSIBLE_DUPLICATE_TARGET)
          break
        default: // ignore
      }
    })
