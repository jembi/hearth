...
console.log(config.getConf('server:fhirVersion'))
...
module.exports = (mongo) => {
  return {
    ...
    validateQueryParams: (resourceType, queryParams, customSupported, callback) => {
      const supportedParams = Object.assign({}, searchParamsMap[resourceType], customSupported)
      ...
      for (const key in queryParams) {
        ...
        // valid param if key found in standardFHIRParams/standardFHIRSearchParams
        const combinedStandardFHIRParams = [].concat(standardFHIRParams).concat(standardFHIRSearchParams)
        if (combinedStandardFHIRParams.indexOf(keyActual) !== -1) {
          continue
        }

        // parameter not supported
        if (!supportedParams[keyActual]) {
