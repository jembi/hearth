...
module.exports = exports = (mongo) => {
  ...
  const lookupPractitionersForOrganization = (ctx, organization, callback) => {
    mongo.getDB((err, db) => {
      if (err) {
        return callback(err)
      }
  ...
  function resolveReference (ctx, reference, callback) {
  ...
  function resolveNestedProperties (ctx, resultList, includeReference, callback) {
    const promises = []
      ...
      _.castArray(property).map((prop) => {
        if (nextProperty[0]) {
          ...
          promises.push(new Promise((resolve, reject) => {
            resolveReference(ctx, prop.reference, (err, data) => {
              if (err) {
                reject(err)
              } else {
                resolve(data)
  ...
  const resolveIncludeParameters = (ctx, item, results) => {
    if (item == null) {
    ...
    const promises = results
      ...
      .map(item => new Promise((resolve, reject) => {
        resolveNestedProperties(ctx, item, itemReference, (err, data) => {
          if (err) {
            reject(err)
          } else {
            resolve(data)
  ...
  function includeResources (ctx, includeParams, results) {
    ...
    _.castArray(includeParams).forEach((item) => {
      promises.push(resolveIncludeParameters(ctx, item, results))
    })
  ...
  function reverseIncludeResources (ctx, results) {
    let revincludes = ctx.query._revinclude
    ...
    if (Array.isArray(revincludes)) {
      ...
      const promises = revincludes.map(revinclude => performReverseInclude(ctx, revinclude, results))
      return Promise.all(promises).then(([...results]) => [].concat(...results))
    } else {
      // Single _revinclude parameter
      return performReverseInclude(ctx, revincludes, results)
    }
  }

  function performReverseInclude (ctx, revinclude, results) {
  ...
  return {
    ...
    bundleResults: (type, entries, total, callingUrl) => {
      ...
      const base = URI(config.getConf("server:publicFhirBase"))
      const entryMap = (base,url) => {
        return (entry) => {
          let search
          if (entry._mpi) {
          ...
          const ret = {
            ...
            fullUrl: base.protocol()+'://'+base.hostname()+':'+base.port()+'/'+url.segment(0)+(url.segment(1) == 'fhir' ? '/fhir' : '' )+'/'+entry.resourceType+'/'+entry.id+'/_history/'+entry.meta.versionId,
            resource: entry,
        ...
        link: [
          {
            ...
            url: base.protocol()+'://'+base.hostname()+':'+base.port()+url.path()+url.search()
          }
        ],
        entry: entries.map(entryMap(base,url))
      }
    },
     ...
     * TODO OHIE-203
     */
    referenceAttachment: (ctx, doc, callback) => {
     ...
     */
    dereferenceAttachment: (ctx, doc, callback) => {
