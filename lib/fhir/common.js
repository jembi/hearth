  ...
  return {
    ...
    util: {
      ...
      tokenToLinkElemMatch: (token) => {
        ...
        const clause = {
          $and: [
            { link: { $elemMatch: { other: { $regex: `^${split[0]}` } } } },
            { link: { $elemMatch: { other: { $regex: `.*${split[1]}.*` } } } } // This query can't be indexed, there are better solutions that require some thought
          ]}
        return clause
      },
      ...
      validateSearchParams: (queryParams, supported, required) => {
        const _supported = _.union(supported, standardFHIRParams)
        if (!_.isEmpty(queryParams) && _.difference(_.keys(queryParams), _supported).length > 0) {
          return `This endpoint only supports the following queries: [${_supported.map((e) => `'${e}'`).join(', ')}]`
        }

        if (required && required.length > 0) {
          const containsRequired = _.intersection(_.keys(queryParams), required).length === required.length
          if (!_.isEmpty(queryParams) && !_.isEmpty(required) && !containsRequired) {
            return `This endpoint has the following required query parameters: [${required.map((e) => `'${e}'`).join(', ')}]`
          }
        }
        return null
      },
