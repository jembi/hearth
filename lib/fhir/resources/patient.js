  ...
  return {
    ...
    searchFilters: (ctx, callback) => {
      ...
      if (queryParams['given']) {
        if (Array.isArray(queryParams['given'])) {
          queryParams['given'].forEach(function (string) {
            query['$and'].push(fhirCommon.util.nameToMongoClause('given', string))
          })
        } else {
      ...
      if (queryParams['family']) {
        if (Array.isArray(queryParams['family'])) {
          queryParams['family'].forEach(function (string) {
            query['$and'].push(fhirCommon.util.nameToMongoClause('family', string))
          })
        } else {
