...
module.exports = (mongo) => {
  ...
  return {
    ...
    read: (req, res) => {
      authorization.authorize('read', req, res, 'user', (resourceType, postInteractionHandler) => {
        mongo.getDB((err, db) => {
          if (err) {
            logger.error(err)
            return res.status(500).send(fhirCommon.internalServerErrorOutcome())
          ...
          c.findOne({email: req.params.email}, {fields: {email: 1, type: 1, resource: 1}}, (err, user) => {
            ...
            postInteractionHandler(req, res, user, (u) => res.send(u))
          })
        })
      })
    },
    ...
    search: (req, res) => {
      authorization.authorize('search', req, res, 'user', (resourceType, postInteractionHandler, searchFilters) => {
        return res.status(500).send(fhirCommon.buildOperationOutcome('information', 'not-supported', 'Not supported'))
      })
    },

    create: (req, res) => {
      authorization.authorize('create', req, res, 'user', (resourceType, postInteractionHandler) => {
        mongo.getDB((err, db) => {
          if (err) {
            logger.error(err)
            return res.status(500).send(fhirCommon.internalServerErrorOutcome())
          ...
          c.findOne({email: req.params.email}, {fields: {email: 1}}, (err, user) => {
            ...
            c.insertOne(req.body, (err) => {
              ...
              postInteractionHandler(req, res, req.body, () => {
    ...
    update: (req, res) => {
      authorization.authorize('update', req, res, 'user', (resourceType, postInteractionHandler) => {
        mongo.getDB((err, db) => {
          if (err) {
            logger.error(err)
            return res.status(500).send(fhirCommon.internalServerErrorOutcome())
