      ...
      authorization.authorize('hierarchy', ctx, resourceType, (err, badRequest) => {
        ...
        hooks.executeBeforeHooks('hierarchy', fhirModule, ctx, resourceType, (err, badRequest) => {
          ...
          mongo.getDB((err, db) => {
            ...
            const viewCollection = resourceType

            mongo.util.debugLog(viewCollection, 'find', {})
            ...
            c.find({}).toArray( (err, results) => {
              ...
              results.forEach( (loc) => {
                if ( loc.partOf && loc.partOf.reference ) {
                  parents["Location/"+loc.id] = loc.partOf.reference
                }
              })

              let parentsIncluded = [ locId ]
              if ( ctx.operation === '$parents' ) {
                let checkId = parents[locId]
                while ( checkId && checkId !== '' && checkId !== locId ) {
                  parentsIncluded.push( checkId )
                  if ( parents[checkId] ) {
                    checkId = parents[checkId]
                  } else {
                    checkId = false
                  }
                }
              ...
              const filters = {
                '$hierarchy': (loc) => {
                  let checkId = (loc.partOf && loc.partOf.reference ? loc.partOf.reference : false)
                  while ( checkId && checkId !== "" && checkId !== locId ) {
                    if ( parents[checkId] ) {
                      checkId = parents[checkId]
                    } else {
                  ...
                  return loc.id === id || checkId === locId 
                },
                '$parents': (loc) => {
                  return parentsIncluded.includes( "Location/" + loc.id )
                }
              }

              if ( filters[ctx.operation] ) {
                let matched = results.filter( filters[ctx.operation] )

                const response = fhirCommon.bundleResults('searchset', matched, matched.length, ctx.url)
                ...
                return callback(null, fhirCommon.buildHTTPOutcome(500, 'error', 'invalid-operation', 'The given operation can\'t be handled by this method: '+ctx.operation))
              }
