...
module.exports = (mongo) => {
  ...
  self.searchFilters = (ctx, callback) => {
      ...
      queryUtils.validateAndParseQueryParams(queryParams, supportedParams, (badRequest, queryObject) => {
        ...
        if (queryObject['_tag']) {
          query['$and'].push(queryUtils.tokenToSystemCodeElemMatch('meta.tag', queryObject['_tag'][constants.NO_MODIFER]))
        }
        ...
        if (queryObject['organization.active']) {
          promises.push(new Promise((resolve, reject) => {
            queryUtils.genChainedParamQuery(ctx, queryObject['organization.active'][constants.NO_MODIFER], 'managingOrganization.reference', 'active', organization, (err, clause) => {
        ...
        if (queryObject['organization.identifier']) {
          promises.push(new Promise((resolve, reject) => {
            queryUtils.genChainedParamQuery(ctx, queryObject['organization.identifier'][constants.NO_MODIFER], 'managingOrganization.reference', 'identifier', organization, (err, clause) => {
              if (err) { return reject(err) }
              query['$and'].push(clause)
              resolve()
            })
        ...
        if (queryObject['organization.name']) {
          for (const modifier in queryObject['organization.name'] ) {
            promises.push(new Promise((resolve, reject) => {
              queryUtils.genChainedParamQuery(ctx, queryObject['organization.name'][modifier], 'managingOrganization.reference', 'name:'+modifier, organization, (err, clause) => {
                if (err) { return reject(err) }
                query['$and'].push(clause)
                resolve()
              })
        ...
        if (queryObject['partof.identifier']) {
          promises.push(new Promise((resolve, reject) => {
            queryUtils.genChainedParamQuery(ctx, queryObject['partof.identifier'][constants.NO_MODIFER], 'partOf.reference', 'identifier', self, (err, clause) => {
              if (err) { return reject(err) }
              query['$and'].push(clause)
              resolve()
            })
        ...
        if (queryObject['partof.name']) {
          for (const modifier in queryObject['partof.name'] ) {
            promises.push(new Promise((resolve, reject) => {
              queryUtils.genChainedParamQuery(ctx, queryObject['partof.name'][modifier], 'partOf.reference', 'name:'+modifier, self, (err, clause) => {
                if (err) { return reject(err) }
                query['$and'].push(clause)
                resolve()
              })
