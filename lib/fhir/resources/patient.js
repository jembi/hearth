...
module.exports = (mongo) => {
  ...
  return {
    ...
    searchFilters: (ctx, callback) => {
      ...
      let supportedParams = ['identifier', 'given', 'family', 'birthDate']
      ...
      if (queryParams['birthDate']) {
        let birthDateClause = { birthDate: { } }
        if (Array.isArray(queryParams['birthDate'])) {
          queryParams['birthDate'].forEach(function (string) {
            birthDateClause = fhirCommon.util.dateToMongoClause(birthDateClause, 'birthDate', string)
          })
        } else {
          birthDateClause = fhirCommon.util.dateToMongoClause(birthDateClause, 'birthDate', queryParams['birthDate'])
        }

        query['$and'].push(birthDateClause)
      }
