...
module.exports = () => {
  ...
  return {
    ...
    util: {
      collapseAndQuery: (query) => ((query['$and'].length === 1) ? query['$and'][0] : query),

      debugLog: (collection, operation, filter, options) => {
        // mongo's own debug logging is quite verbose,
        // so we'll leave its log level as is and handle ourselves
        if (config.getConf().logger.level === 'debug') {
          if (options) {
            logger.debug(`Executing mongo query: ${collection}.${operation}(${JSON.stringify(filter)}, ${JSON.stringify(options)})`)
          } else {
