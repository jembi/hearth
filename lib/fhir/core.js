...
module.exports = (mongo, modules) => {
  ...
  return {
            ...
            c.findOne({ _id: ObjectID(id) }, { fields: { latest: 1 } }, (err, resource) => {
              ...
              postInteractionHandler(ctx, resource, (err, badRequest, data) => {
                ...
                let done = (err, badRequest, data) => {
                  ...
                  let response = fhirCommon.formatResource(data)
        ...
        let processVRead = (err, badRequest) => {
          ...
          mongo.getDB((err, db) => {
            ...
            c.findOne({ _id: ObjectID(id) }, projection, (err, resource) => {
              ...
              if (!resource.latest.meta.versionId === vid && (!resource.history || !resource.history[vid])) {
                return callback(null, fhirCommon.buildHTTPOutcome(404, 'information', 'not-found', 'Not found'))
              }
              ...
              postInteractionHandler(ctx, resource, (err, badRequest, data) => {
                ...
                let done = (err, badRequest, data) => {
                  ...
                  let response = null
                  if (data.latest.meta.versionId === vid) {
                    response = fhirCommon.formatResource(data)
                  } else {
                    response = fhirCommon.formatResourceFromHistory(data, vid)
                  }
     ...
     * @param {ObjectId} [id] The optional id to use during the update
     * @param {CoreCallback} callback
     */
      ...
      authorization.authorize('create', ctx, resourceType, (err, badRequest, postInteractionHandler) => {
        ...
        let processCreate = (err, badRequest) => {
          ...
          mongo.getDB((err, db) => {
            ...
            let c = db.collection(resourceType)
            c.insertOne({ _id: id, latest: newDoc, request: { method: 'POST' }, source: source }, (err, r) => {
              if (err) {
                return callback(err)
              }
              ...
              postInteractionHandler(ctx, newDoc, (err, badRequest) => {
                ...
                let done = (err, badRequest) => {
                  ...
                  callback(null, { httpStatus: 201, location: location, _id: r.insertedId })
                }
              ...
              c.updateOne({ _id: ObjectID(id) }, update, (err) => {
                ...
                postInteractionHandler(ctx, update, (err, badRequest) => {
                  ...
                  let done = (err, badRequest) => {
                    ...
                    callback(null, { httpStatus: 200, location: location, _id: id })
                  }
