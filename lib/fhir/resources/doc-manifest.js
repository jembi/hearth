    ...
    searchFilters: (ctx, callback) => {
      ...
      queryUtils.validateAndParseQueryParams(queryParams, supportedParams, (badRequest, queryObject) => {
        ...
        if (queryObject['related-id']) {
          query['$and'].push({
            'related': {
              $elemMatch: queryUtils.tokenToSystemValueElemMatch('identifier', queryObject['related-id'][constants.NO_MODIFER], true)
            }
          })
        }
        ...
        if (queryObject['author.given']) {
          promises.push(new Promise((resolve, reject) => {
            queryUtils.genChainedParamQuery(queryObject['author.given'][constants.NO_MODIFER], 'author.reference', ['given', 'given', 'name'], [patient, practitioner, relatedPerson], (err, clause) => {
              // console.log(clause)
              if (err) { return reject(err) }
              query['$and'].push(clause)
              resolve()
        ...
        if (queryObject['author.family']) {
          promises.push(new Promise((resolve, reject) => {
            queryUtils.genChainedParamQuery(queryObject['author.family'][constants.NO_MODIFER], 'author.reference', ['family', 'family', 'name'], [patient, practitioner, relatedPerson], (err, clause) => {
              if (err) { return reject(err) }
              query['$and'].push(clause)
              resolve()
            })
