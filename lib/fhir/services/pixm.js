...
module.exports = (mongo) => {
  ...
  return {
    buildPIXmQuery: (ctx, callback) => {
      ...
      queryUtils.validateQueryParams('PIXm', queryParams, customSupportedParams, (badRequest, queryObject) => {
        badRequest = badRequest || queryUtils.validateFullSourceIdentifierToken(queryObject['sourceIdentifier'][constants.NO_MODIFER])

        if (badRequest) {
          return callback(null, fhirCommon.buildHTTPOutcome(400, 'error', 'invalid', badRequest, badRequest))
        }
        ...
        if (!isSourceIdentifierValid(identifier)) {
          return callback(null, fhirCommon.buildHTTPOutcome(400, 'error', 'code-invalid', 'sourceIdentifier Assigning Authority not found', 'sourceIdentifier Assigning Authority not found'))
        }
        ...
        if (!isTargetSystemsValid(queryObject['targetSystem'])) {
          return callback(null, fhirCommon.buildHTTPOutcome(403, 'error', 'code-invalid', 'targetSystem not found', 'targetSystem not found'))
        }
    ...
    afterSearchHook: (ctx, data, callback) => {
      if (data.length > 1) {
        return callback(null, fhirCommon.buildHTTPOutcome(400, 'error', 'invalid', 'query not specific enough, more than one patient found', 'query not specific enough, more than one patient found'))
      }

      if (data.length === 0) {
        callback(null, fhirCommon.buildHTTPOutcome(404, 'error', 'not-found', 'sourceIdentifier Patient Identifier not found', 'sourceIdentifier Patient Identifier not found'))
      }
