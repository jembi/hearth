  ...
  return {
    ...
    searchFilters: (ctx, callback) => {
      ...
      fhirCommon.util.validateAndModifyQueryParams(queryParams, supportedParams, (badRequest, queryObject) => {
        ...
        if (queryObject['_id']) {
          query['$and'].push({'id': queryObject['_id'].value})
        }
        ...
        if (queryObject['telecom']) {
          query['$and'].push(fhirCommon.util.tokenToSystemValueElemMatch('telecom', queryObject['telecom'].value))
        }

        if (queryObject['multipleBirthInteger']) {
          query['$and'].push({ multipleBirthInteger: Number(queryObject['multipleBirthInteger'].value) })
        }
