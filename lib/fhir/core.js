              ...
              fhirCommon.getPagingParams(ctx.query, (err, badRequest, _getpagesoffset, _count) => {
                ...
                c.count(query, projection, (err, total) => {
                  ...
                  c.find(query, projection).skip(_getpagesoffset).limit(_count).toArray((err, results) => {
                    ...
                    const tmpResults = results.slice()

                    // bundle a empty results array to get response structure for afterhooks
                    ...
                    // override entry with original results for use in after hooks - require metadata flags in afterhook
                    response.entry = tmpResults
                    ...
                    hooks.executeAfterHooks('search', fhirModule, ctx, resourceType, response, (err, badRequest, results) => {
                      ...
                      if (results && results.entry) {
                        const response = fhirCommon.bundleResults('searchset', results.entry, results.entry.length, ctx.url)

                        results.entry = response.entry
