...
const processResourceScore = (result) => {
  const matchesMap = {}
  const id = result.id
...
const mpiQuery = (c, query, callback) => {
  ...
  cursor.each(function (err, doc) {
    ...
    if (doc !== null) {
      results.push(processResourceScore(doc))
    } else {
...
const processMatchingQuery = () => {
  ...
  mongo.getDB((err, db) => {
    ...
    mpiQuery(c, query, (err, results) => {
      ...
      respondToParentProcess({ info: `${ctx.workerName} worker successfully completed`, data: matchesMap })
    })
  })
}
