...
module.exports = (mongo) => {
  ...
  const fetchAndFormatMatches = (c, matchingKeys, matchesMap, callback) => {
    c.find({ id: { '$in': matchingKeys } }).toArray((err, results) => {
      ...
      results.forEach((resource) => {
        ...
        // const score = matchesMap[key]._mpi.score
  ...
  const sortFilterLimitAndFormatMatches = (c, matchesMap, count, callback) => {
    ...
    fetchAndFormatMatches(c, limitedKeys, matchesMap, (err, matchesArray) => {
      if (err) {
        return callback(err)
      }
  ...
  const mpiQuery = (c, query, projection, callback) => {
    c.find(query).project(projection).toArray((err, results) => {
      if (err) {
        return callback(err)
      }
      callback(null, results)
  ...
  const exactMatchLookup = (c, query, projection, weight) => {
    const resources = {}
    return new Promise((resolve, reject) => {
      mpiQuery(c, query, projection, (err, results) => {
        if (err) {
          return reject(err)
        }
    ...
    match: (resourceType, queryResource, count, callback) => {
      ...
      mongo.getDB((err, db) => {
        ...
        Object.keys(pathConfigs).forEach((path) => {
          ...
          switch (algorithmTypeMap[algorithm]) {
            case 'exact-match':
              ...
              promises.push(exactMatchLookup(c, query, projection, weight))
              break
        ...
        Promise.all(promises).then((results) => {
          ...
          sortFilterLimitAndFormatMatches(c, matches, count, (err, matchesArray) => {
            if (err) {
              return callback(err)
            }
